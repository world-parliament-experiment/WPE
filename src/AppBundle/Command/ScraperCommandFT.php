...
class ScraperCommandFT extends Command
{
    // the name of the command (the part after "bin/console")
    protected static $defaultName = 'wpe:scrape:folketing';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption('update') === true) {
            $process = new Process('python3 python/scrape_folketing.py');
            ...
            foreach ($NewEntry as $title => $desc) {
                ...
                $desc = preg_replace($url_regex, '<br /> <a href="$0" rel="nofollow">$1</a>', $desc);
                $desc = str_replace("'", "", ($desc));
